...
import numpy as np
import torch
from lib.MetaLearning_HIN import ml_hin
from lib.DataHelper import DataHelper
from tqdm import tqdm
from learning_config import lconfig
from data_config import dconfig
from lib.Config import config_ml as _config
...
def training(model, model_save=True, model_file=None, device='cpu'):
    ...
    batch_size = lconfig['batch_sz']
    num_epoch = lconfig['epoch']
    ...
    for _ in range(num_epoch):  # 20
        ...
        for i in range(num_batch):  # each batch contains some tasks (each task contains a support set and a query set)
            ...
            _loss, _mae, _rmse, _ndcg_5 = model.train_by_batch(support_xs,support_ys,support_mps,
                                                              query_xs,query_ys,query_mps)
            loss.append(_loss)
            mae.append(_mae)
            rmse.append(_rmse)
...
def evaluate(model, state, device='cpu'):
    ...
    for i in range(len(test_data)):  # each task
        _mae, _rmse, _ndcg_5 = model.evaluate(supp_xs_s[i], supp_ys_s[i], supp_mps_s[i],
                                                query_xs_s[i], query_ys_s[i], query_mps_s[i])
        mae.append(_mae)
        rmse.append(_rmse)
        ndcg_at_5.append(_ndcg_5)
...
if __name__ == "__main__":
    ...
    cuda_or_cpu = torch.device("cuda" if lconfig['rtx_on'] else "cpu")
    print (time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()))
    print('-------------------------------\n')

    model_filename = "{}/hml.pkl".format(res_dir)
    data_helper = DataHelper(input_dir, output_dir, _config)
